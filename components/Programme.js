  ...
  return (
    <ProgramDiv>
      <div id="programme" className="container_bg blue_dark">
        <div className="my-6">
          <div className="mb-6 title text-center text-lg md:text-2xl uppercase">
            {programme.title}
          </div>
          <div className="w-full md:w-2/3 mb-6 mx-auto text-center">
            {programme.intro1}
          </div>
          ...
          <div
            className="w-full md:w-2/3 mx-auto mt-8 text-end cursor-pointer underline"
            onClick={() => setShowAll(!showAll)}
          >
            Tout voir
          </div>

          <div className="w-full md:w-2/3 mt-8 mx-auto flex flex-col md:flex-row">
            <div className="w-full md:w-1/2">
              {programme?.modules?.slice(0, 5).map((item, index) => {
                return (
                  ...
                  >
                    <div className={`w-64 lg:w-80 flex items-center`}>
                      <div
                      ...
                      <div className={`title ml-2 lg:text-lg uppercase`}>
                        {item.title}
                      </div>
                    </div>
                    {showDetails.includes(item.lesson) && (
                      <ul className="w-64 lg:w-80 mt-4">
          ...
          <Link
            href={
              "https://www.formation-astrologie-elearning.fr/programme-detaille/"
            }
            target={"_blank"}
          >
